...
from event.eventTypes import EventCreationRequest, EventUpdateRequest, QueryOptions, Id, Event, Ok, SubscriptionStatus
...
async def get_events(query_options: QueryOptions = Depends(),
    ...
    query_options_as_json = jsonable_encoder(query_options)
    print("hi")
    options = {**query_options_as_json,
               "location": location}
    options_without_nones = {key: value for key,
                             value in options.items() if value is not None}
    events = get_events_db(options_without_nones)
